...
function getServiceImage(service: Service, index: number): string {
  ...
  if (name.includes("private sound") || (name.includes("sound") && name.includes("therapy") && !name.includes("unwind") && !name.includes("reset") && !name.includes("restoration"))) {
    return "/lemuria-assets/services/private-sound-primary.jpg";
  }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {services.length === 0 ? (
                        ...
                        (isServicesPage ? services : services.slice(0, 6)).map((service, index) => {
                            const nameLower = (service.title || service.name || "").toLowerCase();
                            const isKinesiology = nameLower.includes("kinesiology");
                            const isPrivateSound = nameLower.includes("private sound") || (nameLower.includes("sound") && nameLower.includes("therapy") && !nameLower.includes("unwind") && !nameLower.includes("reset") && !nameLower.includes("restoration"));
                            return (
                                <motion.div
                                ...
                                >
                                    ...
                                    <div className="absolute inset-0 z-0">
                                        <Image 
                                            ...
                                            className={`object-cover transition-all duration-1000 group-hover:scale-110 ${(isKinesiology || isPrivateSound) ? "group-hover:opacity-0" : ""}`}
                                            quality={95}
                                        />
                                        {isKinesiology && (
                                            <Image 
                                        ...
                                        {isPrivateSound && (
                                            <Image 
                                                src="/lemuria-assets/services/private-sound-hover.jpg" 
                                                alt={`${service.title || service.name || 'Service Image'} - Alternate`}
                                                fill
                                                sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" 
                                                className="object-cover absolute inset-0 transition-all duration-1000 opacity-0 group-hover:opacity-100 scale-105 group-hover:scale-110"
